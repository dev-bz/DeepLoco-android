...
inline void MakeTempDir(string* temp_dirname) {
  ...
  char models[260];
  const path& model =
    std::filesystem::temp_directory_path()/"caffe_test.XXXXXX";
  strcpy(models,model.string().c_str());
  for ( int i = 0; i < CAFFE_TMP_DIR_RETRIES; i++ ) {
    char*done=mkdtemp(models);
    //const path& dir = std::filesystem::unique_path(model).string();
    //bool done = std::filesystem::create_directory(dir);
    if ( done ) {
      *temp_dirname = done;//dir.string();
      return;
    }
  }
