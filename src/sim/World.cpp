...
void cWorld::Init(const tParams &params) {
  ...
  mSimWorld = std::unique_ptr<btDiscreteDynamicsWorld>(
      new btDiscreteDynamicsWorld(mCollisionDispatcher.get(), mBroadPhase.get(), mSolver.get(), mCollisionConfig.get()));
  ...
  if(debugDraw)mSimWorld->setDebugDrawer(debugDraw);
}
void cWorld::Reset() {
...
void cWorld::DebugDraw() {
  showWorld(mSimWorld.get(),mParams.mScale);
}
...
void cWorld::Constrain(cSimObj &obj, const tVector &linear_factor, const tVector &angular_factor) {
  ...
  body->setLinearFactor(btVector3(static_cast<btScalar>(lin_f[0]), static_cast<btScalar>(lin_f[1]), static_cast<btScalar>(lin_f[2])));
  body->setAngularFactor(btVector3(static_cast<btScalar>(ang_f[0]), static_cast<btScalar>(ang_f[1]), static_cast<btScalar>(ang_f[2])));
}
...
void cWorld::SetGravity(const tVector &gravity) {
  ...
  mSimWorld->setGravity(
      btVector3(static_cast<btScalar>(gravity[0] * scale), static_cast<btScalar>(gravity[1] * scale), static_cast<btScalar>(gravity[2] * scale)));
}
...
void cWorld::RayTest(const tVector &beg, const tVector &end, tRayTestResults &results) const {
  ...
  btVector3 bt_beg = scale * btVector3(static_cast<btScalar>(beg[0]), static_cast<btScalar>(beg[1]), static_cast<btScalar>(beg[2]));
  btVector3 bt_end = scale * btVector3(static_cast<btScalar>(end[0]), static_cast<btScalar>(end[1]), static_cast<btScalar>(end[2]));
...
void cWorld::SetPos(const tVector &pos, cSimObj *out_obj) const {
  ...
  btTransform trans = body->getWorldTransform();
  btScalar scale = static_cast<btScalar>(GetScale());
  trans.setOrigin(scale * btVector3(static_cast<btScalar>(pos[0]), static_cast<btScalar>(pos[1]), static_cast<btScalar>(pos[2])));
...
tVector cWorld::GetLinearVelocity(const cSimObj *obj, const tVector &local_pos) const {
  ...
  btVector3 bt_vel =
      body->getVelocityInLocalPoint(scale * btVector3(static_cast<btScalar>(rel_pos[0]), static_cast<btScalar>(rel_pos[1]), static_cast<btScalar>(rel_pos[2])));
  tVector vel = tVector(bt_vel[0], bt_vel[1], bt_vel[2], 0);
  vel /= GetScale();
  return vel;
}
void cWorld::SetLinearVelocity(const tVector &vel, cSimObj *out_obj) const {
  auto &body = out_obj->GetSimBody();
  btScalar scale = static_cast<btScalar>(GetScale());
  body->setLinearVelocity(scale * btVector3(static_cast<btScalar>(vel[0]), static_cast<btScalar>(vel[1]), static_cast<btScalar>(vel[2])));
  // body->activate();
}
...
void cWorld::SetAngularVelocity(const tVector &vel, const cSimObj *obj) const {
  auto &body = obj->GetSimBody();
  body->setAngularVelocity(btVector3(static_cast<btScalar>(vel[0]), static_cast<btScalar>(vel[1]), static_cast<btScalar>(vel[2])));
  // body->activate();
}
...
void cWorld::SetRotation(const tQuaternion &q, cSimObj *out_obj) const {
  auto &body = out_obj->GetSimBody();
  btTransform trans = body->getWorldTransform();
  btQuaternion bt_q = btQuaternion(static_cast<btScalar>(q.x()), static_cast<btScalar>(q.y()), static_cast<btScalar>(q.z()), static_cast<btScalar>(q.w()));
...
void cWorld::ApplyForce(const tVector &force, const tVector &local_pos, cSimObj *out_obj) const {
  auto &body = out_obj->GetSimBody();
  btVector3 bt_force = btVector3(static_cast<btScalar>(force[0]), static_cast<btScalar>(force[1]), static_cast<btScalar>(force[2]));
  btScalar scale = static_cast<btScalar>(GetScale());
  tMatrix3 rot_mat = out_obj->GetLocalToWorldRotMat();
  tVector rel_pos = tVector::Zero();
  rel_pos.segment(0, 3) = rot_mat * local_pos.segment(0, 3);
  btVector3 bt_pos = btVector3(static_cast<btScalar>(rel_pos[0]), static_cast<btScalar>(rel_pos[1]), static_cast<btScalar>(rel_pos[2]));
  bt_force *= scale;
  bt_pos *= scale;
...
void cWorld::ApplyTorque(const tVector &torque, cSimObj *out_obj) const {
  auto &body = out_obj->GetSimBody();
  btScalar scale = static_cast<btScalar>(GetScale());
  body->applyTorque(scale * scale * btVector3(static_cast<btScalar>(torque[0]), static_cast<btScalar>(torque[1]), static_cast<btScalar>(torque[2])));
}
std::unique_ptr<btBoxShape> cWorld::BuildBoxShape(const tVector &box_size) const {
  ...
  std::unique_ptr<btBoxShape> shape = std::unique_ptr<btBoxShape>(new btBoxShape(
      scale * btVector3(static_cast<btScalar>(box_size[0] * 0.5), static_cast<btScalar>(box_size[1] * 0.5), static_cast<btScalar>(box_size[2] * 0.5))));
  return shape;
}
std::unique_ptr<btCapsuleShape> cWorld::BuildCapsuleShape(double radius, double height) const {
  ...
  std::unique_ptr<btCapsuleShape> shape =
      std::unique_ptr<btCapsuleShape>(new btCapsuleShape(static_cast<btScalar>(scale * radius), static_cast<btScalar>(scale * height)));
  return shape;
}
std::unique_ptr<btStaticPlaneShape> cWorld::BuildPlaneShape(const tVector &normal, const tVector &origin) const {
  btVector3 bt_normal = btVector3(static_cast<btScalar>(normal[0]), static_cast<btScalar>(normal[1]), static_cast<btScalar>(normal[2]));
  btVector3 bt_origin = btVector3(static_cast<btScalar>(origin[0]), static_cast<btScalar>(origin[1]), static_cast<btScalar>(origin[2]));
  bt_normal.normalize();
  double scale = GetScale();
...
cWorld::tConstraintHandle cWorld::AddRevoluteConstraint(cSimObj *obj0, cSimObj *obj1, const tJointParams &params) {
  ...
  btVector3 anchor0 =
      scale * btVector3(static_cast<btScalar>(params.mAnchor0[0]), static_cast<btScalar>(params.mAnchor0[1]), static_cast<btScalar>(params.mAnchor0[2]));
  ...
  btVector3 anchor1 =
      scale * btVector3(static_cast<btScalar>(params.mAnchor1[0]), static_cast<btScalar>(params.mAnchor1[1]), static_cast<btScalar>(params.mAnchor1[2]));
...
cWorld::tConstraintHandle cWorld::AddSphericalConstraint(cSimObj *obj0, cSimObj *obj1, const tJointParams &params) {
  ...
  // bullet limits are flipped
  cons->setAngularUpperLimit(
      -btVector3(static_cast<btScalar>(params.mLimLow[0]), static_cast<btScalar>(params.mLimLow[1]), static_cast<btScalar>(params.mLimLow[2])));
  cons->setAngularLowerLimit(
      -btVector3(static_cast<btScalar>(params.mLimHigh[0]), static_cast<btScalar>(params.mLimHigh[1]), static_cast<btScalar>(params.mLimHigh[2])));
  mSimWorld->addConstraint(cons, !params.mEnableAdjacentCollision);
  tConstraintHandle handle;
  handle.mCons = cons;
  return handle;
