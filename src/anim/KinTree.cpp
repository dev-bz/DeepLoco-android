...
// Json keys
const std::string gJointTypeNames[cKinTree::eJointTypeMax] = {"revolute", "planar", "prismatic", "fixed", "spherical", "none"};
const std::string gJointsKey = "Joints";
const std::string gJointDescKeys[cKinTree::eJointDescMax] = {
    "Type",    "Parent",   "AttachX",  "AttachY",  "AttachZ",   "AttachThetaX", "AttachThetaY",  "AttachThetaZ", "LimLow0", "LimLow1",
    "LimLow2", "LimHigh0", "LimHigh1", "LimHigh2", "TorqueLim", "ForceLim",     "IsEndEffector", "DiffWeight",   "Offset"};
const std::string gBodyDefsKey = "BodyDefs";
const std::string gBodyDescKeys[cKinTree::eBodyParamMax] = {"Shape",        "Mass",         "ColGroup", "EnableFallContact",
                                                            "AttachX",      "AttachY",      "AttachZ",  "AttachThetaX",
                                                            "AttachThetaY", "AttachThetaZ", "Param0",   "Param1",
...
const std::string gDrawShapeDescKeys[cKinTree::eDrawShapeParamMax] = {
    "Shape",  "ParentJoint", "AttachX", "AttachY", "AttachZ", "AttachThetaX", "AttachThetaY", "AttachThetaZ",
    "Param0", "Param1",      "Param2",  "ColorR",  "ColorG",  "ColorB",       "ColorA",
};
...
bool cKinTree::LoadBodyDefs(const std::string &char_file, Eigen::MatrixXd &out_body_defs) {
  ...
  succ = Json::parseFromStream(reader, f_stream, &root, &str);
  f_stream.close();
  if (succ) {
    succ = false;
...
bool cKinTree::LoadDrawShapeDefs(const std::string &char_file, Eigen::MatrixXd &out_draw_defs) {
  ...
  Json::Value root;
  Json::CharReaderBuilder reader;
  bool succ = Json::parseFromStream(reader, f_stream, &root, &str);
  f_stream.close();
  if (succ) {
...
tVector cKinTree::CalcBodyPartVel(const Eigen::MatrixXd &joint_mat, const Eigen::MatrixXd &body_defs, const Eigen::VectorXd &state, const Eigen::VectorXd &vel,
                                  int part_id) {
...
cKinTree::eBodyShape cKinTree::GetBodyShape(const Eigen::MatrixXd &body_defs, int part_id) {
  cKinTree::eBodyShape shape = static_cast<eBodyShape>(static_cast<int>(body_defs(part_id, cKinTree::eBodyParamShape)));
  return shape;
}
...
tVector cKinTree::GetBodyAttachTheta(const Eigen::MatrixXd &body_defs, int part_id) {
  tVector attach_theta =
      tVector(body_defs(part_id, eBodyParamAttachThetaX), body_defs(part_id, eBodyParamAttachThetaY), body_defs(part_id, eBodyParamAttachThetaZ), 0);
  return attach_theta;
}
...
bool cKinTree::GetBodyEnableFallContact(const Eigen::MatrixXd &joint_mat, int joint_id) {
  double fall_val = joint_mat(joint_id, eBodyParamEnableFallContact);
  return fall_val != 0;
}
...
void cKinTree::CalcBodyPartRotation(const Eigen::MatrixXd &joint_mat, const Eigen::MatrixXd &body_defs, const Eigen::VectorXd &state, int part_id,
...
void cKinTree::CalcJointWorldTheta(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state, int joint_id, tVector &out_axis, double &out_theta) {
...
tVector cKinTree::CalcWorldVel(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state, const Eigen::VectorXd &vel, int parent_id,
                               const tVector &attach_pt) {
...
tVector cKinTree::CalcWorldAngularVel(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state, const Eigen::VectorXd &vel, int parent_id,
                                      const tVector &attach_pt) {
  cSpAlg::tSpVec sv = cRBDUtil::CalcWorldVel(joint_mat, state, vel, parent_id);
  tVector pos = cKinTree::LocalToWorldPos(joint_mat, state, parent_id, attach_pt);
  cSpAlg::tSpTrans world_to_pt = cSpAlg::BuildTrans(pos);
...
tVector cKinTree::GetJointLimLow(const Eigen::MatrixXd &joint_mat, int joint_id) {
  return tVector(joint_mat(joint_id, cKinTree::eJointDescLimLow0), joint_mat(joint_id, cKinTree::eJointDescLimLow1),
...
tVector cKinTree::GetJointLimHigh(const Eigen::MatrixXd &joint_mat, int joint_id) {
  return tVector(joint_mat(joint_id, cKinTree::eJointDescLimHigh0), joint_mat(joint_id, cKinTree::eJointDescLimHigh1),
...
tVector cKinTree::GetAttachPt(const Eigen::MatrixXd &joint_mat, int joint_id) {
  tVector attach_pt = tVector(joint_mat(joint_id, cKinTree::eJointDescAttachX), joint_mat(joint_id, cKinTree::eJointDescAttachY),
...
tVector cKinTree::GetAttachTheta(const Eigen::MatrixXd &joint_mat, int joint_id) {
  tVector attach_theta = tVector(joint_mat(joint_id, cKinTree::eJointDescAttachThetaX), joint_mat(joint_id, cKinTree::eJointDescAttachThetaY),
...
cKinTree::tBodyDef cKinTree::BuildBodyDef() {
  tBodyDef def;
  def(eBodyParamShape) = static_cast<double>(eBodyShapeBox);
...
void cKinTree::CalcJointPoseDiff(const Eigen::MatrixXd &joint_mat, int joint_id, const Eigen::VectorXd &pose0, const Eigen::VectorXd &pose1,
...
void cKinTree::CalcJointVelDiff(const Eigen::MatrixXd &joint_mat, int joint_id, const Eigen::VectorXd &vel0, const Eigen::VectorXd &vel1,
                                Eigen::VectorXd &out_diff) {
  int param_offset = GetParamOffset(joint_mat, joint_id);
  int param_size = GetParamSize(joint_mat, joint_id);
...
void cKinTree::PoseProcessPose(const Eigen::MatrixXd &joint_mat, Eigen::VectorXd &out_pose) {
...
void cKinTree::MirrorPoseStance(const Eigen::MatrixXd &joint_mat, const std::vector<int> mirror_joints0, const std::vector<int> mirror_joints1,
...
std::string cKinTree::BuildJointMatJson(const Eigen::MatrixXd &joint_mat) {
  ...
  for (int j = 0; j < num_joints; ++j) {
    ...
    std::string joint_json = BuildJointJson(curr_desc);
    if (j != 0) {
      json += ",\n";
    }
    json += joint_json;
...
std::string cKinTree::BuildJointJson(const tJointDesc &joint_desc) {
  std::string json = "";
  json += "{\n";
  for (int i = 0; i < eJointDescMax; ++i) {
