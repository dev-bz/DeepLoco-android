...
public:
	...
	virtual ~cNNSolver();
	virtual std::shared_ptr<caffe::Net<cNeuralNet::tNNData>> GetNet() = 0;
...
public:
	...
	virtual std::shared_ptr<caffe::Net<cNeuralNet::tNNData>> GetNet();
	virtual void ApplySteps(int iters);
	virtual cNeuralNet::tNNData ForwardBackward();
};

template <typename tSolverType>
std::shared_ptr<caffe::Net<cNeuralNet::tNNData>> cCaffeSolver<tSolverType>::GetNet() {
