				...
				for (auto&layer : mNet->layers()) {
					...
					if (b.size() > 0) {
						...
						for (auto&blob : b) {
							...
							fread(data, sizeof(tNNData), count, f);
						}
					}
				}
				fclose(f);
...
void cNeuralNet::EvalBatchSolver(const Eigen::MatrixXd& X, Eigen::MatrixXd& out_Y) const
{
	#if 0
	assert(HasSolver());
	std::shared_ptr<caffe::Net<cNeuralNet::tNNData>> net = GetTrainNet();
...
std::shared_ptr<caffe::Net<cNeuralNet::tNNData>> cNeuralNet::GetTrainNet() const
{
	#if 0
	if (HasSolver())
	{
